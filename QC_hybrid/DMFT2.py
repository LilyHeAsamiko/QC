...
plt.figure()
plt.plot(np.array([H1,H2,H3]).reshape(len(H1),3))
plt.legend(['H1','H2','H3'])
plt.title('Hamiltonians')

plt.figure()
plt.plot(np.real((H1+H2+H3)))
plt.title('Total Hamiltonians')

plt.figure()
plt.pcolor(np.meshgrid([np.sort(tau_t),np.sort(U_t)]),cmap = 'summer')
plt.ylabel('U')
plt.xlabel('tau')
plt.title('colormap of U versus tau')

plt.figure()
plt.contour([tau_t,U_t]) 
plt.ylabel('U')
plt.xlabel('tau')
plt.title('Countour if U versus tau')
...
X = np.sort(tau_t)
Y = np.sort(U_t)
Z = np.meshgrid(H1, H2)#Z, positive
...
X = np.sort(tau_t)
Y = np.sort(U_t)
Z = np.meshgrid(H1, H3)#Z, negative
Z = np.sqrt(np.array(np.real(Z[0]))**2+np.array(np.real(Z[1]))**2)
