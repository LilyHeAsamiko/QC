...
#            qc = QuantumCircuit(qr, cr)
            ...
            for rb_seed,rb_circ_seed in enumerate(rb_circs):
                ...
                rb_circ_transpile = qiskit.transpile(rb_circ_seed, basis_gates=basis_gates)
                print('Simulating seed %d'%rb_seed)
                job = qiskit.execute(rb_circ_transpile, noise_model=noise_model, shots=shots, backend=backend, backend_options={'max_parallel_experiments': 1})
                result_list.append(job.result())
                transpile_list.append(rb_circ_transpile)
            print("Finished Simulating")
...
for l in testidx:
        for i in range(np.shape(W)[0]):
            ...
            for steps in range(50):
                ...
                q = max(max(abs(ones((np.shape(x)))/(1+np.exp(-x*Wtemp)))-label1[l][i]), eps)
                if KL(pred,q) < Dtemp:
                    if i < np.shape(W)[1]:
                        W[i][:] = Wtemp
                    else:
